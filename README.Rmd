...
stations_temp_median <- (met_dt %>% group_by(USAFID) %>% summarise(median_temp = median(temp, na.rm = TRUE)))
stations_wind_speed_median <- (met_dt %>% group_by(USAFID) %>% summarise(median_wind_speed = median(wind.sp, na.rm = TRUE)))
stations_pressure_median <- (met_dt %>% group_by(USAFID) %>% summarise(median_pressure = median(atm.press, na.rm = TRUE)))
...
#print(equal_stations_temp_median)
#print("__________")
#print(equal_stations_wind_speed_median)
#print("__________")
#print(equal_stations_pressure_median)
#print("__________")
...
```{r, warning=FALSE}
states_median <- met_dt %>% group_by(STATE) %>% 
...
merged_df_by_state <- merged_df %>% group_by(STATE) %>% arrange(euclidean) %>% slice(1)
merged_df_by_state[, c("STATE", "USAFID", "euclidean")]
